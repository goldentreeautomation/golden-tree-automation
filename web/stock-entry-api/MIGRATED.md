...
- **Canonical source (지금 수정할 곳):** `business-os/supabase/functions/stock-entry-api`
  (+ 순수 로직은 `business-os/packages/*`)
- **이 디렉터리(`golden-tree-automation/web/stock-entry-api/`):** rollback 참조용으로만 보존. **수정 금지.**
  두 저장소에서 같은 함수를 동시에 고치면 안 된다.
- production 배포: `stock-entry-api` = Business OS v8 (dual-auth)
...
`supabase functions deploy stock-entry-api` 또는 Management API PATCH로 재배포한다.
