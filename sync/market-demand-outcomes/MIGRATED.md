...
- **Canonical source (지금 수정할 곳):** `business-os/supabase/functions/market-demand-outcomes/`
  (+ 순수 로직은 `business-os/packages/business-rules`, 브랜드 상수는 `business-os/packages/config`)
- **이 디렉터리(`golden-tree-automation/sync/market-demand-outcomes/`):** rollback 참조용으로만 보존. **수정 금지.**
