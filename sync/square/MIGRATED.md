...
- **Canonical source (지금 수정할 곳):** `business-os/supabase/functions/square-sync`
  (+ 순수 로직은 `business-os/packages/*`)
- **이 디렉터리(`golden-tree-automation/sync/square/`):** rollback 참조용으로만 보존. **수정 금지.**
  두 저장소에서 같은 함수를 동시에 고치면 안 된다.
- production 배포: `square-sync` = Business OS v19 (FINANCIAL CRITICAL — normalizeOrder 무변경, $0.00 reconciliation)
...
`supabase functions deploy square-sync` 또는 Management API PATCH로 재배포한다.
