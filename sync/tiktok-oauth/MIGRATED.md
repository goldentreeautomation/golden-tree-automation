...
- **Canonical source (지금 수정할 곳):** `business-os/supabase/functions/tiktok-oauth`
  (+ 순수 로직은 `business-os/packages/*`)
- **이 디렉터리(`golden-tree-automation/sync/tiktok-oauth/`):** rollback 참조용으로만 보존. **수정 금지.**
  두 저장소에서 같은 함수를 동시에 고치면 안 된다.
- production 배포: `tiktok-oauth` = Business OS v15 (연결 계정 0)
...
`supabase functions deploy tiktok-oauth` 또는 Management API PATCH로 재배포한다.
