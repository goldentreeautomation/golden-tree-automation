...
- **Canonical source (지금 수정할 곳):** `business-os/supabase/functions/market-demand`
  (+ 순수 로직은 `business-os/packages/*`)
- **이 디렉터리(`golden-tree-automation/sync/market-demand/`):** rollback 참조용으로만 보존. **수정 금지.**
  두 저장소에서 같은 함수를 동시에 고치면 안 된다.
- production 배포: `market-demand` = Business OS v16 (rule-v5 로직 무변경)
...
`supabase functions deploy market-demand` 또는 Management API PATCH로 재배포한다.
