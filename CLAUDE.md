...
메인이 오케스트레이터다. 서브에이전트끼리 직접 호출하지 않는다. 중간 산출물은 `/output/`에 저장하고 경로만 전달한다.

| 요청 유형 | 위임 대상 |
|---|---|
| 테이블·컬럼·Query 함수 | `db-architect` |
| 외부 API 연동·동기화 | `integration-builder` |
| 대시보드 화면 | `frontend-builder` |
| 숫자 검증·마일스톤 판정 | `verifier` |
...
|---|---|---|
| 1 | 레포·CLAUDE.md | 완료 |
| 2 | 빌더 에이전트 4개 정의 | 미착수 (스텁만 존재) |
| 3 | 스키마 확정 | 부분 — 테이블 존재(주문 68,161건)하나 마이그레이션 파일 없음 |
| 4 | Square 동기화 | 완료 — `sync/square/` Edge Function으로 재구현(n8n 미사용). 밀린 구간(8/17~) 채움 |
| 5 | 재실행 안전성 검증 | 완료 — 동일 구간 3회 실행, 행 수 불변 확인 |
| 6 | Square Dashboard 대조 | 부분 완료 — 오류 16건 중 핵심 원인 수정·재검증 완료(Bon Sushi 8/8 $5,451.73 센트 단위 일치). 환불 중복(3번)·페이지네이션(9번) 등 잔여 |
...
**M2 착수** — Discord `/ask` 봇(`0003`), Meta 소셜·광고 동기화(`0004`), Notion 결정기록 신규(`0006`, DB 5개, 동기화 코드는 다음), Regina 시장 수요 예상(`0007`) — 대시보드 왼쪽 패널, 매시간 15분 재계산. 작업 끝날 때마다 표 갱신.
