...
2026-09-04: 1번(소셜 광고 위치 추론이 문자열 매칭) 해결. 실제로 재현해보니 "낮은 우선순위"가 아니라 CozyHaus를 지정한 모든 소셜/광고 질문이 처음부터 조용히 빈 결과를 주던 실제 버그였음 — Bon Sushi와 같은 명시적 location_id 매칭으로 수정. 상세: `docs/decisions/0013`.

2026-08-25: 3번(할인·환불 중복 차감) 해결. n8n 재현이 아니라 새로운 원인을 발견 — Square의 "반품 전용 주문"이 매출 $0으로 잘못 저장되고 있었음(환불이 아예 반영 안 됨, 이중차감이 아니라 무차감). 상세: `docs/decisions/0009`.

2026-08-25: 9번(페이지네이션 완전성) 해결. 전체 기간(70,013건) Square API 원본과 주문 ID 단위로 직접 대조 — 2026-08-13 20:31~23:31 UTC 3시간 구간에서 35건 공백 발견·복구. 상세: `docs/decisions/0010`.
