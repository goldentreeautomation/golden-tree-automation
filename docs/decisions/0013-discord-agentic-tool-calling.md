...
## 다섯 번째 발견: 무한 대기 버그 (타임아웃 없음)

오너 보고: 1시 4분에 물어본 질문이 2시 18분까지(74분) "생각 중"으로 멈춰있었음. 로그 조회는
Supabase 쪽 백엔드 오류로 직접 확인은 못 했으나, 원인이 매우 유력하게 짐작됨 — `fetch()`는
기본적으로 타임아웃이 없다. Gemini나 Supabase REST 호출 중 하나가 응답 없이 연결만 붙잡고
있으면(네트워크 정체, 상대 서버 문제 등) 코드가 영원히 그 응답을 기다린다. `MAX_TOOL_CALLS`로
반복 횟수는 제한해뒀지만, 개별 호출 자체의 대기시간엔 제한이 없었다.

**수정**: `fetchWithTimeout()` 도입(AbortController 기반) — Gemini 호출 30초, Supabase RPC
호출 20초로 강제 제한. 타임아웃되면 일반 에러로 처리되어 기존 에러 핸들링(재시도/사용자 안내)
경로를 그대로 탄다. `discord-bot` v17 배포.

이미 멈춰있던 그 요청(구버전 코드로 실행 중)은 이 배포로 회복되지 않는다 — Discord interaction
webhook은 유효시간이 있어 시간이 많이 지나면 어차피 응답을 못 보낸다. 오너에게 새로 질문해달라고
안내.
