-- 0022_add_stock_status_dispatch.sql — analytics_dispatch에 stock_status 추가.
-- 날짜 범위는 이 analysis엔 의미 없지만(현재 시점 스냅샷) 파라미터 검증 통과를 위해
-- 그대로 받는다(다른 analysis와 동일하게 Gemini가 오늘 날짜를 채워 보낸다).
