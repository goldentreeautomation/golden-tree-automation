-- 0008_post_correlation_multi_day_offset.sql
--
-- 배경: 오너 피드백 — "포스팅 당일 매출"이 아니라 "포스팅 하고 1~3일 뒤에 나타나는 효과"를
-- 보고 싶다. 예: 화요일 포스팅 → 수/목/금 매출이 각각 그 요일의 평소(같은 요일 4주 평균) 대비
-- 어땠는지. 기존 analytics_social_sales_correlation은 포스트 당일(day_offset=0)만 봤음 —
-- analytics_post_item_sales_trend(0005)처럼 day_offset을 0~N으로 확장하되, 각 offset일의
-- "같은 요일 4주 평균" 기준(baseline)을 그 날짜 자체 기준으로 다시 계산한다 — 화요일 포스팅의
-- +1일(수요일)은 수요일 평균과 비교해야지 화요일 평균과 비교하면 안 되기 때문.

create or replace function public.analytics_social_sales_correlation(
  p_start_date date, p_end_date date, p_location_id text default null::text, p_days_after integer default 3
)
 returns table(
   ...
   day_offset integer, business_date date,
...
baseline as (
  -- offset일 자체의 요일 기준 4주 평균 (그 날짜는 제외) — 화요일 포스팅의 +1일(수요일)은
  -- 수요일 평균과 비교해야 한다, 포스팅한 화요일 평균과 비교하면 안 된다
...
select
  td.post_id, td.caption, td.published_at, td.post_business_date, td.location_id,
