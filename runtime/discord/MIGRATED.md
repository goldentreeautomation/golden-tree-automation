...
- **Canonical source (지금 수정할 곳):** `business-os/supabase/functions/discord-bot`
  (+ 순수 로직은 `business-os/packages/*`)
- **이 디렉터리(`golden-tree-automation/runtime/discord/`):** rollback 참조용으로만 보존. **수정 금지.**
  두 저장소에서 같은 함수를 동시에 고치면 안 된다.
- production 배포: `discord-bot` = Business OS v33
...
`supabase functions deploy discord-bot` 또는 Management API PATCH로 재배포한다.
